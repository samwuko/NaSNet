...
class CrossNet(object):
  ...
  def huber_loss(self, labels, predictions, delta=1.0):
    ...
    residual = tf.reduce_sum(tf.abs(predictions - labels), 1, keep_dims=True)
  ...
  def build_model(self, data, is_training=True):
    ...
    self.feat_aerial = models.pixelnet_a(self.image_aerial, self.num_classes,
        is_training=is_training, batch_norm=self.batch_norm)
    self.feat_ground = models.pixelnet_g(self.image_ground, self.num_classes,
        is_training=is_training, batch_norm=self.batch_norm)#
    misc.pprint(self.feat_aerial.get_shape().as_list()) # print the feature size
    misc.pprint(self.feat_ground.get_shape().as_list())  # print the feature size

    if is_training:
      feat_aerial_small = self.feat_aerial
      feat_ground_small = self.feat_ground
    else:
      feat_aerial_small = tf.image.resize_bilinear(self.feat_aerial, self.szs.before_transf)
      feat_ground_small = tf.image.resize_bilinear(self.feat_ground, self.szs.after_transf)
    ...
    if is_training:
      ...
      #huber loss
      with tf.name_scope("Loss"):
        self.loss = self.huber_loss(feat_ground_small, self.feat_aerial2ground)
    ...
    self.prob_ground = tf.nn.softmax(self.feat_ground)
